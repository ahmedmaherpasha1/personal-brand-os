...
export default function AppLayout() {
  ...
  const currentLabel = navItems.find((item) => location.pathname.startsWith(item.path))?.label || 'Personal Brand OS';

  return (
    <div className="flex min-h-screen bg-surface">
      {/* Desktop Sidebar */}
      <aside className="hidden md:flex w-64 bg-surface-container-low flex-col flex-shrink-0">
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-64 bg-surface-container-low flex flex-col transform transition-transform duration-300 ease-in-out md:hidden ${
          sidebarOpen ? 'translate-x-0' : '-translate-x-full'
        }`}
      >
        <SidebarContent user={user} logout={logout} onNavClick={closeSidebar} />
      </aside>

      {/* Main content area */}
      <div className="flex-1 flex flex-col min-w-0">
        {/* Top bar */}
        <header className="h-16 flex items-center px-4 md:px-8 bg-surface-container-lowest shadow-ambient">
          {/* Mobile hamburger */}
          <button
            onClick={() => setSidebarOpen(true)}
            className="md:hidden mr-3 p-1.5 rounded-btn text-on-surface-variant hover:bg-surface-container-high/50 transition-colors duration-200"
            aria-label="Open menu"
          >
            <span className="material-symbols-outlined text-[24px]">menu</span>
          </button>
          <h2 className="font-display font-semibold text-on-surface">{currentLabel}</h2>
        </header>
